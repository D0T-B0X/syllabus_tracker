...
import 'package:syllabus_tracker/model/topics_list-model.dart';
...
Future<void> loadCourse() async {
  ...
  final jsonString = await rootBundle.loadString('assets/data/subject_data.json');
  ...
  data.forEach((code, value) {
    ...
    if(saved != null) {
      ...
      _topicsList[code] = {
        for (var topic in course.topics) topic: false,
      };
    }
  });
  notifyListeners();
  ...
  // function - 2
  void toggleTopic(String code, String topic, bool selected, String userID) async {
...
Future<void> UpdateTotalCount(String userID) async {
  ...
  await Supabase.instance.client.from('user_data').update({
    'topics_completed': total}).eq('user_id', userID);
}
